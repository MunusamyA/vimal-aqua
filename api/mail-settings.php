...
{
    if ($user['branch_id'] !== null && (int) $user['branch_id'] > 0) {
        return positive_id($user['branch_id'], 'branch_id');
    }
    if ((int) $user['role_type'] === 2) {
        return null;
    }
    json_error('Your user account has no assigned branch.', 403);
}
...
{
    // Scope is determined by login; no manual branch switching.
    return [];
}
...
if ($method === 'POST' || $method === 'PUT') {
    ...
    if (($data['action'] ?? '') === 'reset') {
        if ($branchId === null) {
            $delete = db()->prepare('DELETE FROM email_settings WHERE branch_id IS NULL');
            $delete->execute();
        } else {
            $delete = db()->prepare('DELETE FROM email_settings WHERE branch_id = :branch_id');
            $delete->execute([':branch_id' => $branchId]);
        }
        ...
        json_success('Mail settings reset successfully.', ['branch_id' => $branchId]);
    }
