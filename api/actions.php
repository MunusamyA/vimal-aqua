...
function validate_action_group($value): int
{
    ...
    if (!isset(action_group_names()[$groupId])) {
        json_error('Select a valid action group.', 422, ['group_id' => 'Invalid action group.']);
    }
    return $groupId;
}
...
function clean_action_name($value): string
{
    ...
    if ($name === '' || strlen($name) > 120) {
        json_error('Enter a valid action name.', 422, ['action_name' => 'Action name is required and must be within 120 characters.']);
    }
    return $name;
}
...
function ensure_unique_action_name(string $name, int $excludeId = 0): void
{
    ...
    $stmt->execute($params);
    if ($stmt->fetchColumn()) {
        json_error('Action name already exists.', 409, ['action_name' => 'Use a unique action name.']);
    }
}
...
function clean_action_purpose($value): ?string
{
    ...
    if (strlen($purpose) > 255) {
        json_error('Purpose is too long.', 422, ['purpose' => 'Purpose must be within 255 characters.']);
    }
    return $purpose;
}
...
if ($method === 'GET') {
    ...
    if (isset($_GET['datatable']) && (int) $_GET['datatable'] === 1) {
        ...
        $params = [];
        if ($search !== '') {
            $where = ' WHERE CAST(id AS CHAR) LIKE :search OR action_name LIKE :search OR purpose LIKE :search';
            $params[':search'] = '%' . $search . '%';
        }

        $total = (int) db()->query('SELECT COUNT(*) FROM permission_actions')->fetchColumn();
        $countStmt = db()->prepare('SELECT COUNT(*) FROM permission_actions' . $where);
        $countStmt->execute($params);
        $filtered = (int) $countStmt->fetchColumn();

        $columns = ['id', 'action_name', 'group_id', 'purpose', 'status', 'sort_order'];
        $orderColumn = (int) ($_GET['order'][0]['column'] ?? 0);
        $orderDir = strtolower((string) ($_GET['order'][0]['dir'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $orderBy = $columns[$orderColumn] ?? 'id';

        $sql = 'SELECT id, action_name, purpose, group_id, sort_order, status, created_at, updated_at
                FROM permission_actions' . $where .
               ' ORDER BY ' . $orderBy . ' ' . $orderDir . ', id ASC LIMIT :start, :length';
        $stmt = db()->prepare($sql);
        ...
        foreach ($stmt->fetchAll() as $row) {
            ...
            $rows[] = [
                ...
                'group_name' => $groups[(int) $row['group_id']] ?? ('Group ' . (int) $row['group_id']),
...
if ($method === 'POST') {
    ...
    $data = request_data();
    require_fields($data, ['action_name' => 'Action name is required.', 'group_id' => 'Action group is required.']);
    ...
    json_success('Permission action created successfully.', ['action_id' => $id], 201);
}
...
if ($method === 'PUT') {
    ...
    $user = require_platform_user();
    $data = request_data();
    require_fields($data, ['id', 'action_name', 'group_id']);
    ...
    $groupId = validate_action_group($data['group_id']);
    $sortOrder = isset($data['sort_order']) ? max(0, (int) $data['sort_order']) : (int) $old['sort_order'];
...
if ($method === 'PATCH') {
    ...
    $access = require_permission('actions.php', $status === 1 ? ACTION_ACTIVATE : ACTION_DEACTIVATE);
    $old = action_record($id);

    if ($id <= ACTION_MANAGE_MENUS && $status === 0) {
        json_error('Built-in actions 1 to 54 cannot be deactivated.', 409);
    }

    db()->prepare(
        'UPDATE permission_actions SET status = :status, updated_by = :updated_by, updated_at = NOW() WHERE id = :id'
    )->execute([
        ':status' => $status,
        ':updated_by' => (int) $user['id'],
        ':id' => $id,
    ...
    json_success($status === 1 ? 'Permission action activated.' : 'Permission action deactivated.');
}
