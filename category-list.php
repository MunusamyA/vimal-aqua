...
<div class="page-head">
    <div>
        <h1>Category List</h1>
        <p>Manage categories for the current branch.</p>
    </div>
...
<div class="kpi-grid">
    <div class="card kpi-card"><div class="kpi-icon blue"><i data-lucide="folder-tree"></i></div><div><div class="kpi-label">Total Categories</div><div class="kpi-value" id="kpiTotal">0</div></div></div>
    <div class="card kpi-card"><div class="kpi-icon green"><i data-lucide="circle-check-big"></i></div><div><div class="kpi-label">Active Categories</div><div class="kpi-value" id="kpiActive">0</div></div></div>
    <div class="card kpi-card"><div class="kpi-icon orange"><i data-lucide="circle-off"></i></div><div><div class="kpi-label">Inactive Categories</div><div class="kpi-value" id="kpiInactive">0</div></div></div>
...
<div class="card table-card">
    <div class="card-header">
        <div class="form-row">
            <div class="field col-8">
                ...
                <input id="masterSearch" type="text" autocomplete="off" placeholder="Search categories...">
            </div>
    ...
    <table id="masterTable" class="display data-table" style="width:100%">
        <thead><tr><th>Code</th><th>Category</th><th>Description</th><th>Status</th><th>Manage</th></tr></thead>
    </table>
</div>

<?php require __DIR__ . '/model/category-form.php'; ?>
...
(function ($, window, document) {
    ...
    async function refreshStats() {
        try {
            ...
            statsRows = result.data.categories || [];
            updateStats();
        } catch (error) {
            statsRows = [];
            updateStats();
    ...
    var table = AppDataTable.init("#masterTable", {
        ...
        searching: true,
        searchDelay: 350,
        appSearchPlaceholder: "Search Categories...",
        appLoaderText: "Loading categories...",
        pageLength: 10,
        ...
        autoWidth: false,
        buttons: [
            {extend:"copyHtml5",text:"Copy",title:"Category List",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1,2,3]}},
            {extend:"csvHtml5",text:"CSV",title:"Category List",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1,2,3]}},
            {extend:"excelHtml5",text:"Excel",title:"Category List",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1,2,3]}},
            {extend:"pdfHtml5",text:"PDF",title:"Category List",orientation:"landscape",pageSize:"A4",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1,2,3]}},
            {extend:"print",text:"Print",title:"Category List",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1,2,3]}}
        ],
        ajax: function (data, callback) {
            ...
            App.api("api/category.php?" + params.toString()).then(function (result) {
                ...
                App.showError(error, "Unable to load categories.");
                callback({draw:data.draw,recordsTotal:0,recordsFiltered:0,data:[]});
            });
        },
        columns: [
            {data:"category_code",defaultContent:"-",render:function(v,t){return t==="display"?escapeHtml(v||"-"):v;}},
            ...
            {data:"status",render:function(v,t){return statusHtml(v,t);}},
            {
                ...
                render:function(data,type,row){
                    ...
                    if (Number(row.status) === 1 && has(allowedActions, ACTION_DEACTIVATE)) {
                        html += '<button type="button" class="table-icon-action danger js-status" data-id="'+Number(row.id)+'" data-status="2" title="Deactivate Category" aria-label="Deactivate Category"><i data-lucide="circle-off"></i></button>';
                    } else if (Number(row.status) !== 1 && has(allowedActions, ACTION_ACTIVATE)) {
                        html += '<button type="button" class="table-icon-action js-status" data-id="'+Number(row.id)+'" data-status="1" title="Activate Category" aria-label="Activate Category"><i data-lucide="circle-check"></i></button>';
                    }
                    return html || '<span class="muted">View only</span>';
                }
            }
        ],
        language:{emptyTable:"No categories found.",zeroRecords:"No matching categories found."},
    ...
    addButton.addEventListener("click",function(){
        if (!has(allowedActions, ACTION_CREATE) || !window.AppCategoryForm) return;
    ...
    document.addEventListener("click",function(event){
        ...
        if(editButton && has(allowedActions,ACTION_UPDATE) && window.AppCategoryForm) {
            AppCategoryForm.openEdit(Number(editButton.getAttribute("data-id")||0),{
                apiUrl:"api/category.php",
                onSaved:function(){table.ajax.reload(null,false);refreshStats();}
            });
            return;
        ...
        App.api("api/category.php",{
            ...
            refreshStats();
        }).catch(function(error){App.showError(error,"Unable to update Category status.");})
