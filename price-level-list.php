...
<div class="page-head">
    <div>
        <h1>Price Level List</h1>
        <p>Manage price levels for the current branch.</p>
    </div>
    <button class="btn btn-primary" id="addButton" type="button" style="display:none">
        <i data-lucide="plus"></i>Add Price Level
    </button>
</div>

<div class="kpi-grid">
    <div class="card kpi-card"><div class="kpi-icon blue"><i data-lucide="badge-indian-rupee"></i></div><div><div class="kpi-label">Total Price Levels</div><div class="kpi-value" id="kpiTotal">0</div></div></div>
    <div class="card kpi-card"><div class="kpi-icon green"><i data-lucide="circle-check-big"></i></div><div><div class="kpi-label">Active Price Levels</div><div class="kpi-value" id="kpiActive">0</div></div></div>
    <div class="card kpi-card"><div class="kpi-icon orange"><i data-lucide="circle-off"></i></div><div><div class="kpi-label">Inactive Price Levels</div><div class="kpi-value" id="kpiInactive">0</div></div></div>
...
<div class="card table-card">
    <div class="card-header">
        <div class="form-row">
            <div class="field col-8">
                ...
                <input id="masterSearch" type="text" autocomplete="off" placeholder="Search price levels...">
            </div>
    ...
    <table id="masterTable" class="display data-table" style="width:100%">
        <thead><tr><th>Price Level</th><th>Status</th><th>Manage</th></tr></thead>
    </table>
</div>

<?php require __DIR__ . '/model/price-level-form.php'; ?>
...
(function ($, window, document) {
    ...
    function updateStats() {
        ...
        var matching = statsRows.filter(function(row) {
            if (q && String(row.price_level_name || "").toLowerCase().indexOf(q) === -1) return false;
    ...
    async function refreshStats() {
        try {
            var result = await App.api("api/price-level.php");
            statsRows = result.data.price_levels || [];
        } catch (error) {
            statsRows = [];
        }
        updateStats();
    ...
    var table = AppDataTable.init("#masterTable", {
        ...
        searchDelay: 350,
        appSearch: false,
        appLoaderText: "Loading price levels...",
        pageLength: 10,
        ...
        autoWidth: false,
        buttons: [
            {extend:"copyHtml5",text:"Copy",title:"Price Level List",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1]}},
            {extend:"csvHtml5",text:"CSV",title:"Price Level List",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1]}},
            {extend:"excelHtml5",text:"Excel",title:"Price Level List",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1]}},
            {extend:"pdfHtml5",text:"PDF",title:"Price Level List",orientation:"landscape",pageSize:"A4",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1]}},
            {extend:"print",text:"Print",title:"Price Level List",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1]}}
        ],
        ajax: function (data, callback) {
            ...
            App.api("api/price-level.php?" + params.toString()).then(function (result) {
                ...
                App.showError(error, "Unable to load price levels.");
            ...
            {data:"status",render:function(v,t){return statusHtml(v,t);}},
            {
                ...
                render:function(data,type,row){
                    ...
                    if (has(allowedActions, ACTION_UPDATE)) {
                        html += '<button type="button" class="table-icon-action js-edit" data-id="'+Number(row.id)+'" title="Edit Price Level" aria-label="Edit Price Level"><i data-lucide="pencil"></i></button>';
                    }
                    if (Number(row.status) === 1 && has(allowedActions, ACTION_DEACTIVATE)) {
                        html += '<button type="button" class="table-icon-action danger js-status" data-id="'+Number(row.id)+'" data-status="2" title="Deactivate Price Level" aria-label="Deactivate Price Level"><i data-lucide="circle-off"></i></button>';
                    } else if (Number(row.status) !== 1 && has(allowedActions, ACTION_ACTIVATE)) {
                        html += '<button type="button" class="table-icon-action js-status" data-id="'+Number(row.id)+'" data-status="1" title="Activate Price Level" aria-label="Activate Price Level"><i data-lucide="circle-check"></i></button>';
                    }
                    return html || '<span class="muted">View only</span>';
                }
            }
        ],
        language:{emptyTable:"No price levels found.",zeroRecords:"No matching price levels found."},
    ...
    addButton.addEventListener("click",function(){
        if (!has(allowedActions, ACTION_CREATE) || !window.AppPriceLevelForm) return;
        AppPriceLevelForm.openCreate({
            apiUrl:"api/price-level.php",
    ...
    document.addEventListener("click",function(event){
        ...
        if(editButton && has(allowedActions,ACTION_UPDATE) && window.AppPriceLevelForm) {
            AppPriceLevelForm.openEdit(Number(editButton.getAttribute("data-id")||0),{
                apiUrl:"api/price-level.php",
                onSaved:function(){table.ajax.reload(null,false);refreshStats();}
            });
            return;
        ...
        App.api("api/price-level.php",{
            ...
            refreshStats();
        }).catch(function(error){App.showError(error,"Unable to update Price Level status.");})
