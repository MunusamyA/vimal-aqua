...
<div class="card table-card">
    <div class="card-header">
        <div class="form-row">
            <div class="field col-8">
                ...
                <input id="masterSearch" type="text" autocomplete="off" placeholder="Search subcategories...">
            </div>
    ...
    <table id="masterTable" class="display data-table" style="width:100%">
        <thead><tr><th>Code</th><th>Category</th><th>Subcategory</th><th>Status</th><th>Manage</th></tr></thead>
    </table>
</div>
...
(function ($, window, document) {
    ...
    var table = AppDataTable.init("#masterTable", {
        ...
        searching: true,
        searchDelay: 350,
        appSearchPlaceholder: "Search Subcategories...",
        ...
        autoWidth: false,
        buttons: [
            {extend:"copyHtml5",text:"Copy",title:"Subcategory List",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1,2,3]}},
            {extend:"csvHtml5",text:"CSV",title:"Subcategory List",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1,2,3]}},
            {extend:"excelHtml5",text:"Excel",title:"Subcategory List",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1,2,3]}},
            {extend:"pdfHtml5",text:"PDF",title:"Subcategory List",orientation:"landscape",pageSize:"A4",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1,2,3]}},
            {extend:"print",text:"Print",title:"Subcategory List",action:AppDataTable.serverSideExportAction,exportOptions:{columns:[0,1,2,3]}}
        ],
        ...
        columns: [
            {data:"subcategory_code",defaultContent:"-",render:function(v,t){return t==="display"?escapeHtml(v||"-"):v;}},
            {data:"category_name",defaultContent:"-",render:function(v,t){return t==="display"?escapeHtml(v||"-"):v;}},
            {data:"subcategory_name",defaultContent:"-",render:function(v,t){return t==="display"?escapeHtml(v||"-"):v;}},
            {data:"status",render:function(v,t){return statusHtml(v,t);}},
            {
                ...
                render:function(data,type,row){
                    ...
                    if (Number(row.status) === 1 && has(allowedActions, ACTION_DEACTIVATE)) {
                        html += '<button type="button" class="table-icon-action danger js-status" data-id="'+Number(row.id)+'" data-status="2" title="Deactivate Subcategory" aria-label="Deactivate Subcategory"><i data-lucide="circle-off"></i></button>';
                    } else if (Number(row.status) !== 1 && has(allowedActions, ACTION_ACTIVATE)) {
                        html += '<button type="button" class="table-icon-action js-status" data-id="'+Number(row.id)+'" data-status="1" title="Activate Subcategory" aria-label="Activate Subcategory"><i data-lucide="circle-check"></i></button>';
                    }
                    return html || '<span class="muted">View only</span>';
                }
            }
        ],
        language:{emptyTable:"No subcategories found.",zeroRecords:"No matching subcategories found."},
    ...
    addButton.addEventListener("click",function(){
        if (!has(allowedActions, ACTION_CREATE) || !window.AppSubcategoryForm) return;
    ...
    document.addEventListener("click",function(event){
        ...
        if(editButton && has(allowedActions,ACTION_UPDATE) && window.AppSubcategoryForm) {
            AppSubcategoryForm.openEdit(Number(editButton.getAttribute("data-id")||0),{
                apiUrl:"api/subcategory.php",
                onSaved:function(){table.ajax.reload(null,false);}
            });
            return;
        ...
        }).catch(function(error){App.showError(error,"Unable to update Subcategory status.");})
